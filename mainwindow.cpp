...
void MainWindow::on_pushButton_clicked()
{
    ...
    pay.initialize(
        ...
        "2D641F75283524A8BB2EF089E83863A041C0B3FA"
        );

    // 构建订单
    SQL_Order order;
    order.orderId = QString("ORDER%1").arg(QDateTime::currentSecsSinceEpoch());
    order.productName = "测试商品";
    order.totalPrice = 0.01;  // 1分钱
    order.quantity = 1;
    order.user = "test_user";
    order.note = "测试订单";

    // 直接调用支付接口，返回二维码图片
    QPixmap qrCode = pay.requestPayment(order, "https://yourdomain.com/notify", "127.0.0.1");
    ...
    if (!qrCode.isNull()) {
        ...
        // 显示关闭按钮
        QPushButton *closeBtn = new QPushButton("关 闭", dialog);
        ...
        qDebug() << "❌ 支付失败";
        QMessageBox::warning(this, "失败", "支付请求失败");
    }
}
